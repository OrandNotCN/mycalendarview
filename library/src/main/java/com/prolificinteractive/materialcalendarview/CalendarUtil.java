...
public class CalendarUtil {
     ...
     * @return String
     */
    public String getChineseDate(int y, int m, int d) {
        Log.e(">>>>>>>>>>getCda:"," "+y+" "+m+" "+d);
        CalendarUtil c = new CalendarUtil();
        c.setGregorian(y, m, d);
        c.computeChineseFields();
        c.computeSolarTerms();
        int cdDate = c.getChineseDate();
        if (cdDate == 0) return "oo";
        int cdMonth = c.getChineseMonth();
        if (cdMonth < 1 || cdMonth > 29)
            cdMonth = 1;
        Log.e(">>>>>>>>>>getCda:"," "+cdMonth+" "+cdDate);
        return y + monthOfAlmanac[cdMonth - 1] + daysOfAlmanac[cdDate - 1];
    }
