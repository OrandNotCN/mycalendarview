...
public class CalendarUtil2 {  
     ...
     * @author liu 2015-1-5 
     */  
    public static long lunarToSolar(String lunarDate) throws Exception{
        ...
        checkLunarDate(lunarYear, lunarMonth, lunarDay, isLeapYear);
        ...
        if(isLeapYear & leapMonth != lunarMonth){
            throw(new Exception("您输入的闰月标志有误！"));  
        }  
  
        //当年没有闰月或月份早于闰月或和闰月同名的月份  
        if(leapMonth==0|| (lunarMonth < leapMonth) || (lunarMonth==leapMonth && !isLeapYear)){
     ...
     * @author liu 2015-1-5 
     */
    public static String solarToLunar(Date myDate){
        int i;
        int temp = 0;
        int lunarYear;
        int lunarMonth; //农历月份  
...
//        return "阴历："+lunarYear+"年"+(leapMonthFlag&(lunarMonth==leapMonth)?"闰":"")+lunarMonth+"月"+lunarDay+"日";
        return lunarYear+"年"+monthOfAlmanac[lunarMonth-1]+daysOfAlmanac[lunarDay];
    }

    public static String getDate(long timestamp,boolean isLunar){
        if (isLunar){
            return "农历"+getDate(timestamp);
        }else{
     ...
     * @author liu 2015-1-5
     */
    public static String getLunarDay(Date myDate ){
        int i;
        int temp = 0;
        int lunarYear;
        int lunarMonth; //农历月份
        ...
        lunarMonth = i;
        lunarDay = offset;
        return daysOfAlmanac[lunarDay-1];
    }
